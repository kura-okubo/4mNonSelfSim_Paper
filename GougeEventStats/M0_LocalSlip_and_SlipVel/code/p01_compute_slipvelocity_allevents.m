...
% 2025.1.25 update for the new merged catalog
% 2025.1.28 update for applying two-way low-pass filter instead of movmean
% for the simplicity of processing. Deprecate saving the AE waveforms.
...
figdir="../figure/debug_p01_compute_slipvelocity/slipvelcontour";
if ~exist(figdir) mkdir(figdir); end
figdir_debug="../figure/debug_p01_compute_slipvelocity/timehistory";
if ~exist(figdir_debug) mkdir(figdir_debug); end
addpath("../../../utils/matlabcode_biax_v03");

datadir="../data/p01_slipandslipvel";
...
for event_ind = 1:length(T.event_id)
    ...
    % % % bandpass filter.
    % Update 2025.1.28: We apply low-pass filter
    ...
    f_nyq = Fs_slip/2;
    bandpass_order = 4;
    [b,a] = butter(bandpass_order, (1/lowpass_winlen)/f_nyq, "low");
    % freqz(b, a, [], Fs_slip);
    % ax = findall(gcf, 'Type', 'axes');
    % set(ax, 'XScale', 'log');
    Dmat_event_filtered = filtfilt(b,a,Dmat_event_offsetremoved);
    ...
    foname=datadir+sprintf("/FB03-%03d_slipvelocity_event%03d.mat", expr_id, event_id);
