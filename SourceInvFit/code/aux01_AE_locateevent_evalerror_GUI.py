...
fi_catalog="../../../Experiments/DetectEvent/data/p06_visual_pick_gougeevents.csv"
columns = ["expr_id", "event_id", "event_loc", "picktime", "event_type", "rupturetype", "foreshock_id", "doublecheck"]
...
foreshock_eventset2 = [3, 16, 17, 30, 51, 52, 58, 61, 77, 91, 99, 106] # to evaluate error in the location of event

for i, df_evt in df_expr.iterrows():
    # if not df_evt['foreshock_id']==20:
    if not df_evt['foreshock_id'] in foreshock_eventset2:
    ...
    print(f"start processing eventid={df_evt['event_id']}: foreshock_id={df_evt['foreshock_id']:04d}")
    ...
    # store meta data
    outputdir = os.path.join(outdir, "arrivalpick_evallocerror")
    eventoutdir = outputdir+"/{:04d}".format(i)
    ...
    st_obs.foreshock_id = df_evt['foreshock_id'] # index
