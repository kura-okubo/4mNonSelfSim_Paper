...
def eval_biax_4m_eventloc(tpick, st, channel_loc, Vrng, PickNumStation=6, gridsize=1e-3, datacase="", vfixed_id=False):
    ...
    Xrng=np.linspace(X0-0.12, X0+0.12, int(np.round(0.12/gridsize)) + 1, endpoint=True) #[m] 2021/12/22 narrow down area
    Xrng = np.delete(Xrng, np.where(Xrng<0.003));
    Xrng = np.delete(Xrng, np.where(Xrng>L-0.003));
    Nx=len(Xrng);

    Yrng=np.linspace(-W/2, W/2, int(np.round(0.1/gridsize)) + 1, endpoint=True) #[m] 
    Ny=len(Yrng);
    ...
    for n1 in tqdm(range(Nx), desc="Grid searching:"):
        for n2 in range(Ny):
            ...
            for n3 in range(Nv):
                Erdat[n3]=np.var(pick - 1e3*Dmat/Vrng[n3], ddof=0); #[ms]
    ...
    # debug
    if os.path.exists("../../data/DEBUG_pick_arrival"):
    	np.savez('../../data/DEBUG_pick_arrival/Erdat_{}.npz'.format(datacase), Erdat_all=Erdat_all, Xrng=Xrng, Yrng=Yrng, Vrng=Vrng)
