...
% 
    ...
    if (T.eventtype(event_id) == 2) || (T.eventtype(event_id) == 3)
        AE_xlimit = [0, 200];
    else
        AE_xlimit = [0, 60];
    end
    ...
    for i = 1:Nsensors_AE
        ...
        if ii<16
            lc = [0, 0, 0.6, 0.4]; %[0, 0.4470, 0.7410]; %blue
        else
            lc = [0, 0, 0.6, 0.4]; %[0.8500, 0.3250, 0.0980]; %red
        end
...
%     pan on;
    ...
    figdir = sprintf("../figure/p04_plotevent_FB03_%03d_AEandStrain", expr_id);
    if ~exist(figdir) mkdir(figdir); end


    figname = sprintf(figdir+"/p04_plot_event_FB03_%03d_event%02d_AEandStrain.png", expr_id, event_id);

    if ifSavefig
        exportgraphics(fig, figname, 'Resolution',250);
    end
